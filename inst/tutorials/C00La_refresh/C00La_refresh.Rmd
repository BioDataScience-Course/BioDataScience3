...
```{r, include=FALSE}
# Code pour les focntion speedy, tidy and fast
# Code pour le remaniement
sselect <- collapse::fselect
tselect <- dplyr::select
...
-   Si vous réalisez un groupement (tgroup_by() ou sgroup_by()) suivi d'un résumé des observations (tsummarise() ou ssummarise()), le tableau initial n'est pas modifié =\> `|>`
...
```{r filter_h3-check}
grade_code("Parfait ! Vous avez employé correctemetn la fonction sfilter().", "Il semble que vous avez commis une erreur. n'hésitez pas à revoir la section 5.4.2 de SDD I.")
```
...
Il faut impérativement vérifier l'homoscédasticité ainsi que la distribution normale des résidus lors de la réalisation d'une ANOVA.
...
```{r crabs_acp_prep}
# Calcul des nouvelles variables
crabs_red <- fmutate(crabs, front = front/width, length = length/width, 
    rear = rear/width, depth = depth/width, sp_sex = paste0(species, sex))
